...
public class JwtService {
  ...
  public String generateJWT(Account account) {
    ...
    if (account instanceof Owner) claims.put(PERMISSIONS_CLAIM_KEY, PERMISSIONS_OWNER);
      ...
      throw new IllegalArgumentException(
          "Specified account is neither an owner, employee, nor a customer");
    return doGenerateToken(claims, account.getEmail());
  }

  /**
   * This method is a helper method which performs the generation of the JWT given payloads and an
   * email.
   *
   * @author Philippe Sarouphim Hochar.
  ...
  /**
   * This method extracts the "permissions" field from the body of the JWT and returns it. It only
   * does so if the JWT is valid.
   *
   * @author Philippe Sarouphim Hochar.
   * @param token JWT.
  ...
  public boolean getAccountPermission(String token, String email) {
    ...
    String sub =
        (String) Jwts.parser().setSigningKey(secret).parseClaimsJws(token).getBody().get("subject");
